...
class ConclusionPipeline:
    """
    Conclusion sentence feedback pipeline.

    Reads paragraph blocks from each `conc_para.docx`, analyzes each paragraph in
    fixed-size batches, and appends feedback text to `fb.docx`.
    """
    ...
    def run_pipeline(self) -> dict[str, Any]:
        triplets = self._conclusion_triplets()
        if not triplets:
            return {
                "document_count": 0,
                "task_count": 0,
        ...
        def _run_all_batches() -> dict[str, Any]:
            ...
            type_print("Running conclusion sentence analysis in batches of 4.", color=Color.BLUE)
            task_count = 0
            success_count = 0
            failure_count = 0
            ...
            for idx in range(0, len(tasks), 4):
                ...
                type_print(
                    f"Conclusion analyzer batch: {idx + 1} to {idx + len(batch_tasks)}",
                    color=Color.GREEN,
                )
                batch_inputs = [task["paragraph"] for task in batch_tasks]
                batch_result = self.llm_task_service.analyze_conclusion_sentence_parallel(
                ...
                for task, output in zip(batch_tasks, batch_result["outputs"]):
                    ...
                    if isinstance(output, Exception):
                        item["errors"].append(
                            f"paragraph {task['paragraph_idx']}: {output}"
                        )
                        continue
                    ...
                    if not feedback:
                        item["errors"].append(
                            f"paragraph {task['paragraph_idx']}: empty feedback"
                        )
                        continue
    ...
    def _prepare_tasks(
        ...
        type_print("Loading conc_para.docx files for conclusion analysis.", color=Color.BLUE)
    ...
    def _conclusion_triplets(self) -> list["DiscoveredPathTriplet"]:
        return (
