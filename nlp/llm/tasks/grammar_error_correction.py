...
def build_correction_requests(text_tasks: Sequence[str]) -> list[ChatRequest]:
    return [
        ChatRequest(
...
async def run_parallel_grammar_correction(
    llm_service: "LlmService",
    app_cfg: "AppConfigShape",
    text_tasks: Sequence[str],
    max_concurrency: int | None = None,
) -> dict[str, Any]:
    requests_ = build_correction_requests(text_tasks)
